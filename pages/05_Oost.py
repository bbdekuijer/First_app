...
# Bereken percentages per partij voor Oost
oost_row = stadsdeel_data[stadsdeel_data['Stadsdeel'] == 'Oost'].iloc[0]
for partij in partij_columns:
    stadsdeel_data.loc[stadsdeel_data['Stadsdeel'] == 'Oost', partij + ' %'] = (oost_row[partij] / oost_row['Total Votes']) * 100

# Functie om taartdiagrammen te maken voor Oost
...
# Genereer een taartdiagram voor Oost en toon het
img_bytes = create_pie_chart(oost_row, oost_row['Stadsdeel'])
st.image(img_bytes, caption=f"Stemverdeling in {oost_row['Stadsdeel']}", use_column_width=True)
