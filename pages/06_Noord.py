...
# Bereken percentages per partij voor Noord
noord_row = stadsdeel_data[stadsdeel_data['Stadsdeel'] == 'Noord'].iloc[0]
for partij in partij_columns:
    stadsdeel_data.loc[stadsdeel_data['Stadsdeel'] == 'Noord', partij + ' %'] = (noord_row[partij] / noord_row['Total Votes']) * 100

# Functie om taartdiagrammen te maken voor Noord
...
# Hoofd Streamlit-app
st.title("Stemverdeling van de stemmen in stadsdeel Noord")

# Genereer een taartdiagram voor Noord en toon het
img_bytes = create_pie_chart(noord_row, noord_row['Stadsdeel'])
st.image(img_bytes, caption=f"Stemverdeling in {noord_row['Stadsdeel']}", use_column_width=True)
