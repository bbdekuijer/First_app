...
# Bereken percentages per partij voor Zuidoost
zuidoost_row = stadsdeel_data[stadsdeel_data['Stadsdeel'] == 'Zuidoost'].iloc[0]
for partij in partij_columns:
    stadsdeel_data.loc[stadsdeel_data['Stadsdeel'] == 'Zuidoost', partij + ' %'] = (zuidoost_row[partij] / zuidoost_row['Total Votes']) * 100

# Functie om taartdiagrammen te maken voor Zuidoost
...
# Hoofd Streamlit-app
st.title("Stemverdeling van de stemmen in stadsdeel Zuidoost")

# Genereer een taartdiagram voor Zuidoost en toon het
img_bytes = create_pie_chart(zuidoost_row, zuidoost_row['Stadsdeel'])
st.image(img_bytes, caption=f"Stemverdeling in {zuidoost_row['Stadsdeel']}", use_column_width=True)
