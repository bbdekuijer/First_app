...
# Bereken percentages per partij voor Zuid
zuid_row = stadsdeel_data[stadsdeel_data['Stadsdeel'] == 'Zuid'].iloc[0]
for partij in partij_columns:
    stadsdeel_data.loc[stadsdeel_data['Stadsdeel'] == 'Zuid', partij + ' %'] = (zuid_row[partij] / zuid_row['Total Votes']) * 100

# Functie om taartdiagrammen te maken voor Zuid
...
# Hoofd Streamlit-app
st.title("Stemverdeling van de stemmen in stadsdeel Zuid")

# Genereer een taartdiagram voor Zuid en toon het
img_bytes = create_pie_chart(zuid_row, zuid_row['Stadsdeel'])
st.image(img_bytes, caption=f"Stemverdeling in {zuid_row['Stadsdeel']}", use_column_width=True)
