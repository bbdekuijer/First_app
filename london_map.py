...
# Filepaths for each year from GitHub
data_files = {
...
def load_yearly_data(year):
    ...
    metro_data['total_usage'] = metro_data['Entry_Week'] + metro_data['Entry_Saturday'] + metro_data['Entry_Sunday']
    top_25_stations = metro_data[['Station', 'total_usage']].sort_values(by='total_usage', ascending=False).head(25)
    return top_25_stations

# Streamlit app title
st.title('Top 25 Busiest Metro Stations in London (2007-2017)')

# Dropdown or slider to select the year
selected_year = st.slider('Select Year', min_value=2007, max_value=2017, step=1)

# Load the data for the selected year
top_25_stations = load_yearly_data(selected_year)

# Plot the top 25 busiest stations
st.subheader(f'Top 25 Busiest Metro Stations in {selected_year}')
fig, ax = plt.subplots(figsize=(12, 8))
ax.barh(top_25_stations['Station'], top_25_stations['total_usage'], color='steelblue')
ax.set_xlabel('Total Metro Usage')
ax.set_ylabel('Stations')
ax.set_title(f'Top 25 Busiest Stations in London - {selected_year}')
ax.invert_yaxis()  # Invert y-axis to have the busiest station on top
st.pyplot(fig)
